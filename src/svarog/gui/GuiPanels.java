	...
	public ByteBuffer bottomPanel(int windowWidth) { 
		if(bottomPanel != null) {
			if(this.bottomPanel.getWidth() == 1) {
				ByteBuffer pixels = BufferUtils.createByteBuffer(0);
				if(windowWidth*bottomPanel.getHeight()*4 > 0) {
					pixels = BufferUtils.createByteBuffer(windowWidth*bottomPanel.getHeight()*4);
	...
	public ByteBuffer rightPanel(int windowHeight) { 
		if(rightImage != null) {
			if(this.rightImage.getHeight() == 1) {
				ByteBuffer pixels = BufferUtils.createByteBuffer(0);
				if(windowHeight*rightImage.getWidth()*4 > 0) {
					pixels = BufferUtils.createByteBuffer(windowHeight*rightImage.getWidth()*4);
